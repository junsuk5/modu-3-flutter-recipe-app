...
void main() {
  group('레시피 레포지토리 테스트 : ', () {
    ...
    setUpAll(() async {
      mockClient = MockClient((request) async {
        if (request.url.toString() ==
            '${HttpUrl.baseUrl}${RecipeDataSourceImpl.endpoint}') {
    ...
    test('데이터가 제대로 List<Recipe> 형태로 들어와야한다.', () async {
      final result = await repository.findDatas();

      switch (result) {
        case Success<List<Recipe>, RecipeErrorEnum>():
          final List<Recipe> recipes = result.success;

          expect(recipes.length, 10);
          expect(recipes.first.chef, 'Chef John');
          expect(recipes.last.chef, 'Paul Hollywood');
        case Error<List<Recipe>, RecipeErrorEnum>():
          fail('해당 테스트는 성공되어야한다.');
      }
    });
    test('에러가 발생되어야한다.', () async {
      mockClient = MockClient((request) async {
        if (request.url.toString() == '${HttpUrl.baseUrl}') {
          return http.Response(
            jsonEncode({
              "recipes": [
                {
      ...
      switch (result) {
        case Success<List<Recipe>, RecipeErrorEnum>():
          fail('해당 테스트는 실패되어야한다.');
        case Error<List<Recipe>, RecipeErrorEnum>():
          expect(result.error, RecipeErrorEnum.networkerror);
      }
    });
  });
}
